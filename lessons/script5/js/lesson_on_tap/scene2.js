...
class scene2 extends Phaser.Scene{
	...
	preload(){
		this.load.image('bg2', 'assets/background2_1.png');
	...
	create(){
		...
		// add text on screen
		var chars = [];
		var x_char = screen.x/2 - 40;
		var y_char = screen.y - 50;
		
		var list = loops['loop' + loop]
		console.log(list[4] == ' ')
		for (var i in list) {
			if (lesson == 15)
				chars.push(this.add.text(x_char, y_char, (i == 0 || list[i - 1] == ' ') ? list[i]: list[i].toLowerCase(), {fontSize:70, fontFamily: 'Comic Sans MS', color:'#000000'})
								.setOrigin(0.5)
								.setAlpha(0.5)
				)
			else 
		...
		this.input.keyboard.on('keydown', function (event) {			
			...
			if (event.key.toUpperCase() == current_key) {
				...
				if (lesson == 15)
					chars[current_index] = (this.add.text(chars[current_index].x, chars[current_index].y, (list[current_index-1] == ' ' || current_index == 0 ?  current_key : current_key.toLowerCase()), {fontSize:70, fontFamily: 'Comic Sans MS', color:'red'}).setOrigin(0.5));
				else
			...
			else if (event.key.toUpperCase() == list[current_index - 1]  || event.key == 'Shift' || event.key == 'CapsLock') {
				// console.log('trung')
			} 
			else {	// type wrong
				voice_wrong.play();
